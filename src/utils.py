"""Shared paths, data-quality checks, and the Parquet export.
...
Run with: python -m src.utils (after python -m src.transform_star_schema)
"""
...
def run_all_checks(con: duckdb.DuckDBPyConnection) -> list[CheckResult]:
    if not table_exists(con, FACT_TABLE):
        raise RuntimeError(
            "fact_sales is missing. Run python -m src.ingest then "
...
def build_report(con: duckdb.DuckDBPyConnection, exported: dict[str, int]) -> str:
    ...
    lines += [
        ...
        "### By category",
        "",
        _table(loss_making_by(con, "category")),
        "### By sub-category",
        "",
        _table(loss_making_by(con, "sub_category"), limit=20),
        "### By region",
        "",
        _table(loss_making_by(con, "region")),
        "### By segment",
        "",
        _table(loss_making_by(con, "segment")),
        "## Check detail",
        "",
    ]
